//TODO: объявление переменных

// let a = 'Pete';
// const b = 'John';
// var c = 'Igor';

// const newArr = [...arr];

// console.log(newArr, arr)


// Принимает один или несколько элементов, которые добавятся в массив и возвращает длинну массива
// const word = 'Alona'
// const res = arr.push(word, 'dobavili');
// console.log(res);

// 3 параметра 
// 1 - индекс по которому начинает изменяться массив
// 2 - число, показывающее количество старых удаляемых из массива элементов
// 3 - Необязательные параметры. Добавляемые к массиву элементы. Если вы не укажете никакого элемента
// Возвращает новый массив вырезанных элементов
// const arrSplice = arr.splice(0,2, 'Marina', "array"
// arrSplice.push('Hello')
// console.log(arrSplice, arr)

// Добавляет элементы в начало массива и ВОЗВРАЩАЕТ ДЛИНУ МАССИВА
// const length = arr.unshift('some string', 'unshift');
// console.log(arr);

// удаляет первый элемент из массива и возвращает его значение. Этот метод изменяет длину массива
// const shiftArr = arr.shift();
// console.log(shiftArr, arr);

// Метод pop() удаляет последний элемент из массива и возвращает его значение. Если массив пуст - вернет undefined

// chaining

// const arr = ['John', 'Igor', 'Sasha', 'Chuvak', 'Bob'];


// const res = arr.splice(0, 3, 'chaining').push('chaining2')

// console.log(res)

// console.log(res)

// console.log(res)
// arr
// .push('some words')
// .shift()

// console.log(arr);
